<template>
  <main class="navbar">
    <section class="navbar-brand">
      ...
      <h1 class="text-dark-50 text-xl font-semibold lg:text-3xl">BEEBIG</h1>
      <button
        @click="showMenu = true"
        class="active:scale-95 duration-300 md:hidden"
        type="button"
      >
        <i class="fa fa-bars text-dark-50 text-2xl"></i>
      </button>
    </section>
    ...
    <section :class="showMenu ? 'right-0' : '-right-full'" class="navbar-menu">
      <ul class="navbar-list">
        <span @click="showMenu = false" class="md:hidden"
          ><i class="fa fa-times text-dark-50"></i
        ></span>
        <template v-for="(menu, x) in menus" :key="x">
          <li
            :class="
              menuActive === menu.name ? 'border-b-4 border-blue-400' : ''
            "
            class="text-dark-50 font-medium duration-300"
          >
            <a @click="menuActive = menu.name" :href="menu.to">{{
              menu.name
            }}</a>
          </li>
        </template>
